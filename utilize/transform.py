...
def random_split(X, y, M, test_size = 0.2, val_size = 0.2, random_seed = 42):
    '''
    Split the dataset into training and test set given the test users' index in uuid_list
    '''
    indice = np.random.RandomState(seed = random_seed).permutation(X.shape[0])
    split_point_test = int(X.shape[0]*test_size)
    split_point_val = int(X.shape[0]*val_size)
    indice_test = indice[:split_point_test]
    indice_val = indice[split_point_test:split_point_test+split_point_val]
    indice_train = indice[split_point_test+split_point_val:]

    X_train = X[indice_train, :]
    y_train = y[indice_train, :]
    M_train = M[indice_train, :] 

    X_test = X[indice_test, :]
    y_test = y[indice_test, :]
    M_test = M[indice_test, :]
